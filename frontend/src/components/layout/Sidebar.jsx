...
const Sidebar = () => {
  ...
  return (<>
    ...
    <div className={`fixed h-full z-50 ${theme === 'dark' ? 'bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 border-gray-700 shadow-black' : 'bg-gradient-to-b from-orange-50/95 via-amber-50/95 to-orange-100/95 border-orange-200/30 shadow-2xl'} backdrop-blur-xl border-r transition-all duration-300 ease-in-out ${isMobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} ${isCollapsed && window.innerWidth >= 1024 ? 'w-32 pt-16' : 'w-64'} overflow-y-auto sidebar-scrollbar scrollbar-thin ${theme === 'dark' ? 'scrollbar-thumb-orange-700 scrollbar-track-gray-800' : 'scrollbar-thumb-orange-500 scrollbar-track-orange-100'}`}>
      <button className="hidden lg:block fixed top-4 right-4 z-50 bg-orange-200 hover:bg-orange-300 text-orange-800 rounded-full p-2 shadow-md transition-all duration-300" onClick={() => setIsCollapsed((prev) => !prev)} aria-label={isCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}>
        {isCollapsed ? (<svg className="w-8 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 12h16m-7-7l7 7-7 7" /></svg>) : (<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 12H4m7-7l-7 7 7 7" /></svg>)}
      </button>
      ...
      <nav className="flex-1 p-3 space-y-1">{filteredNavigation.map((item, index) => { const isActive = location.pathname === item.href; const itemKey = `nav-${index}`; return (<div key={item.name} className="relative" onMouseEnter={() => setHoveredItem(itemKey)} onMouseLeave={() => setHoveredItem(null)}>
